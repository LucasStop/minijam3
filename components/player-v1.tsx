  ...
  ({ onShoot }, ref) => {
    ...
    useFrame((state, delta) => {
      ...
      if (controls.w) {
        ...
        meshRef.current.getWorldDirection(forwardVector);
        // Move a nave nessa direção
        meshRef.current.position.add(forwardVector.multiplyScalar(moveSpeed));
      }
      ...
      if (controls.space) {
        ...
        if (currentTime - lastShotTime.current > shootCooldown) {
          ...
          meshRef.current.getWorldDirection(shootDirection);
