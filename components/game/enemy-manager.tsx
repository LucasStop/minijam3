...
export function EnemyManager({ difficulty = 1 }: EnemyManagerProps) {
  ...
  const spawnBasicEnemy = (viewport: any) => {
    ...
    const spawnY =
      Math.random() * (viewport.height * 0.3) + viewport.height * 0.2;
    const spawnZ = -25;
  ...
  const spawnFastEnemy = (viewport: any) => {
    ...
    const spawnX =
      side * (viewport.width * 0.6 + Math.random() * viewport.width * 0.3);
  ...
  const spawnHeavyEnemy = (viewport: any) => {
    ...
    const spawnY =
      viewport.height * 0.3 + Math.random() * viewport.height * 0.2;
    const spawnZ = -30;
